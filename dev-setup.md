...
STEP 1：Dify ワークスペース確認
    ↓
STEP 2：Tavily API キー取得・Dify に登録
    ↓
STEP 3：Google Cloud セットアップ（Sheets API）
    ↓
STEP 4：Google スプレッドシート作成
    ↓
STEP 5：Dify ワークフロー構築
    ↓
STEP 6：動作テスト
```
...
1. https://cloud.dify.ai にログイン（長谷さんから招待済みのアカウントで）
2. 左上のワークスペース名を確認（長谷さんのワークスペースになっているか）
3. 右上のアカウントメニュー → 「Billing」または「Credits」でクレジット残高を確認
   - 長畑さんが入れた3000円分（ホスト型クレジット）が表示されているはず
4. 「Studio」→「Create App」→「Workflow」でアプリを作れることを確認
...
2. メールアドレスで無料登録
3. ダッシュボードで API キーをコピー（`tvly-xxxx...` という形式）
...
2. 「Tavily」を検索して選択
3. API キーを貼り付けて保存
4. テスト検索「英検 ニュース」を実行して結果が返ってくることを確認

---

## STEP 3：Google Cloud セットアップ
...
6. ダウンロードした JSON ファイルを安全な場所に保存（`.gitignore` に追加・Git には絶対に入れない）
...
1. スプレッドシート右上「共有」
2. STEP 3 でメモした `client_email` を入力
3. 権限：「編集者」で共有
...
[LLM（Claude Haiku）]
  プロンプト：
    - システムプロンプト：コンテンツ生成の指示（カテゴリ・フォーマット・禁止事項）
    - ユーザープロンプト：収集した情報 + 今日の日付 + 生成指示
  出力：Instagram 5候補 + note 5候補（JSON形式）
    ↓
[Code ノード or Template]（JSONをスプレッドシート書き込み用に整形）
    ↓
