...
> 当初は Tavily Search でリアルタイム情報収集を行う予定でしたが、権限問題で V1 導入が困難でした。次に Dify 標準 Web Search を検討しましたが、現バージョンでは外部 API キーが必須で無料選択肢もなくなっているため除外しました。V1 はリアルタイム検索なしで LLM の学習データをもとに生成します。V2 以降で Tavily を導入しリアルタイム情報収集を実現します。
